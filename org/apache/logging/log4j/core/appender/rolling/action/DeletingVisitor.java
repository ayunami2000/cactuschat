...
public class DeletingVisitor extends SimpleFileVisitor<Path>
{
    ...
    public FileVisitResult visitFile(final Path file, final BasicFileAttributes attrs) throws IOException {
        for (final PathCondition pathFilter : this.pathConditions) {
            ...
            if (!pathFilter.accept(this.basePath, relative, attrs)) {
                DeletingVisitor.LOGGER.trace("Not deleting base={}, relative={}", new Object[] { this.basePath, relative });
                return FileVisitResult.CONTINUE;
            }
        }
        if (this.isTestMode()) {
            DeletingVisitor.LOGGER.info("Deleting {} (TEST MODE: file not actually deleted)", new Object[] { file });
        }
        else {
            this.delete(file);
        }
    ...
    protected void delete(final Path file) throws IOException {
        DeletingVisitor.LOGGER.trace("Deleting {}", new Object[] { file });
