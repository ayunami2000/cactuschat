...
public final class RoutingAppender extends AbstractAppender
{
    ...
    private RoutingAppender(final String name, final Filter filter, final boolean ignoreExceptions, final Routes routes, final RewritePolicy rewritePolicy, final Configuration config, final PurgePolicy purgePolicy) {
    ...
    public void start() {
        for (final Route route : this.routes.getRoutes()) {
            if (route.getAppenderRef() != null) {
                final Appender appender = this.config.getAppender(route.getAppenderRef());
                if (appender != null) {
