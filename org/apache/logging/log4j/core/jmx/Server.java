...
public final class Server
{
    ...
    private static ExecutorService createExecutor() {
        ...
        return async ? Executors.newFixedThreadPool(1, new DaemonThreadFactory("log4j2.jmx.notif")) : null;
    }
    ...
    public static void reregisterMBeansAfterReconfigure(final MBeanServer mbs) {
        ...
        try {
            ...
            Server.LOGGER.trace("Reregistering MBeans after reconfigure. Selector={}", new Object[] { selector });
            final List<LoggerContext> contexts = selector.getLoggerContexts();
            int i = 0;
            for (final LoggerContext ctx : contexts) {
                Server.LOGGER.trace("Reregistering context ({}/{}): '{}' {}", new Object[] { ++i, contexts.size(), ctx.getName(), ctx });
    ...
    private static void unregisterStatusLogger(final String contextName, final MBeanServer mbs) {
        final String pattern = "org.apache.logging.log4j2:type=%s,component=StatusLogger";
    ...
    private static void unregisterContextSelector(final String contextName, final MBeanServer mbs) {
        final String pattern = "org.apache.logging.log4j2:type=%s,component=ContextSelector";
    ...
    private static void unregisterAllMatching(final String search, final MBeanServer mbs) {
        try {
            ...
            if (found.isEmpty()) {
                Server.LOGGER.trace("Unregistering but no MBeans found matching '{}'", new Object[] { search });
            }
            else {
                Server.LOGGER.trace("Unregistering {} MBeans: {}", new Object[] { found.size(), found });
            }
            ...
            Server.LOGGER.error("Could not unregister MBeans for " + search, ex);
        }
    }
    ...
    private static void register(final MBeanServer mbs, final Object mbean, final ObjectName objectName) throws InstanceAlreadyExistsException, MBeanRegistrationException, NotCompliantMBeanException {
        Server.LOGGER.debug("Registering MBean {}", new Object[] { objectName });
