...
public class PluginRegistry
{
    ...
    private Map<String, List<PluginType<?>>> decodeCacheFiles(final ClassLoader loader) {
        ...
        try {
            ...
            if (resources == null) {
                PluginRegistry.LOGGER.info("Plugin preloads not available from class loader {}", new Object[] { loader });
            }
            else {
        ...
        for (final Map.Entry<String, Map<String, PluginEntry>> outer : cache.getAllCategories().entrySet()) {
            ...
            for (final Map.Entry<String, PluginEntry> inner : outer.getValue().entrySet()) {
                ...
                try {
                    ...
                    PluginRegistry.LOGGER.info("Plugin [{}] could not be loaded due to missing classes.", new Object[] { className, e });
                }
                catch (VerifyError e2) {
                    PluginRegistry.LOGGER.info("Plugin [{}] could not be loaded due to verification error.", new Object[] { className, e2 });
                }
            }
        }
        ...
        PluginRegistry.LOGGER.debug("Took {} seconds to load {} plugins from {}", new Object[] { numFormat.format(seconds), pluginCount, loader });
        return newPluginsByCategory;
    }
    ...
    public Map<String, List<PluginType<?>>> loadFromPackage(final String pkg) {
        ...
        final DecimalFormat numFormat = new DecimalFormat("#0.000000");
        final double seconds = (endTime - startTime) * 1.0E-9;
        PluginRegistry.LOGGER.debug("Took {} seconds to load {} plugins from package {}", new Object[] { numFormat.format(seconds), resolver.getClasses().size(), pkg });
