...
public class PluginManager
{
    ...
    public void collectPlugins(final List<String> packages) {
        ...
        PluginManager.LOGGER.debug("PluginManager '{}' found {} plugins", new Object[] { this.category, newPlugins.size() });
    ...
    private static void mergeByName(final Map<String, PluginType<?>> newPlugins, final List<PluginType<?>> plugins) {
        ...
        for (final PluginType<?> pluginType : plugins) {
            ...
            if (existing == null) {
                ...
                PluginManager.LOGGER.warn("Plugin [{}] is already mapped to {}, ignoring {}", new Object[] { key, existing.getPluginClass(), pluginType.getPluginClass() });
            }
        }
    }
