...
public class PluginBuilder implements Builder<Object>
{
    ...
    public Object build() {
        this.verify();
        try {
            PluginBuilder.LOGGER.debug("Building Plugin[name={}, class={}].", new Object[] { this.pluginType.getElementName(), this.pluginType.getPluginClass().getName() });
            ...
            PluginBuilder.LOGGER.error("Unable to inject fields into builder class for plugin type {}, element {}.", new Object[] { this.clazz, this.node.getName(), e });
        }
        try {
            ...
            PluginBuilder.LOGGER.error("Unable to invoke factory method in class {} for element {}.", new Object[] { this.clazz, this.node.getName(), e });
            return null;
        }
    }
    ...
    private static Builder<?> createBuilder(final Class<?> clazz) throws InvocationTargetException, IllegalAccessException {
        for (final Method method : clazz.getDeclaredMethods()) {
            if (method.isAnnotationPresent(PluginBuilderFactory.class) && Modifier.isStatic(method.getModifiers()) && TypeUtil.isAssignable(Builder.class, method.getGenericReturnType())) {
                ReflectionUtil.makeAccessible(method);
                final Builder<?> builder = (Builder<?>)method.invoke(null, new Object[0]);
                return builder;
            }
        }
        return null;
    ...
    private void injectFields(final Builder<?> builder) throws IllegalAccessException {
        ...
        for (final Field field : fields) {
            log.append((log.length() == 0) ? (this.simpleName(builder) + "(") : ", ");
    ...
    private Object[] generateParameters(final Method factory) {
        ...
        for (int i = 0; i < annotations.length; ++i) {
            ...
            final String argName = "arg[" + i + "](" + this.simpleName(value2) + ")";
            for (final ConstraintValidator<?> validator : validators) {
    ...
    private void verifyNodeChildrenUsed() {
        ...
        if (!this.pluginType.isDeferChildren() && !children.isEmpty()) {
            for (final Node child : children) {
                ...
                PluginBuilder.LOGGER.error("{} has no parameter that matches element {}", new Object[] { start, child.getName() });
            }
        }
    }
