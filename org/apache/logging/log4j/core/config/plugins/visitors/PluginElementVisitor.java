        ...
        if (this.conversionType.isArray()) {
            ...
            for (final Node child : node.getChildren()) {
                ...
                if (name.equalsIgnoreCase(childType.getElementName()) || this.conversionType.isAssignableFrom(childType.getPluginClass())) {
                    ...
                    if (childObject == null) {
                        PluginElementVisitor.LOGGER.error("Null object returned for {} in {}.", new Object[] { child.getName(), node.getName() });
                    }
                    else {
            ...
            if (!values.isEmpty() && !this.conversionType.isAssignableFrom(values.get(0).getClass())) {
                PluginElementVisitor.LOGGER.error("Attempted to assign attribute {} to list of type {} which is incompatible with {}.", new Object[] { name, values.get(0).getClass(), this.conversionType });
                return null;
            }
