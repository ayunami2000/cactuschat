...
{
    private static final Map<Object, List<Object>> values;
    
    private static void register(final Enum<?> key, final Object value) {
        if (!MagicValues.values.containsKey(key)) {
            MagicValues.values.put(key, new ArrayList<Object>());
        }
        MagicValues.values.get(key).add(value);
    }
    
    public static <T> T key(final Class<T> keyType, final Object value) {
        for (final Object key : MagicValues.values.keySet()) {
            if (keyType.isAssignableFrom(key.getClass())) {
                for (final Object val : MagicValues.values.get(key)) {
                    if (val == value || val.equals(value)) {
                        return (T)key;
                    }
                    if (String.class.isAssignableFrom(val.getClass()) && String.class.isAssignableFrom(value.getClass())) {
                        final String string1 = (String)val;
                        final String string2 = (String)value;
    ...
    public static <T> T value(final Class<T> valueType, final Object key) {
        if (MagicValues.values.containsKey(key)) {
            for (final Object val : MagicValues.values.get(key)) {
    ...
    static {
        values = new HashMap<Object, List<Object>>();
        ...
        for (final BuiltinSound sound : BuiltinSound.values()) {
