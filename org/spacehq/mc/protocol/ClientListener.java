...
public class ClientListener extends SessionAdapter
{
    ...
    public void packetReceived(final PacketReceivedEvent event) {
        ...
        if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
            if (event.getPacket() instanceof EncryptionRequestPacket) {
                ...
                event.getSession().setCompressionThreshold(event.getPacket().getThreshold());
            }
        }
        else if (protocol.getSubProtocol() == SubProtocol.STATUS) {
            if (event.getPacket() instanceof StatusResponsePacket) {
                final ServerStatusInfo info = event.getPacket().getInfo();
                ...
                final long time = System.currentTimeMillis() - event.getPacket().getPingTime();
            ...
            if (event.getPacket() instanceof ServerKeepAlivePacket) {
                ...
                event.getSession().disconnect(event.getPacket().getReason().getFullText());
            }
    ...
    public void connected(final ConnectedEvent event) {
        ...
        if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
            ...
            event.getSession().send(new HandshakePacket(210, event.getSession().getHost(), event.getSession().getPort(), HandshakeIntent.LOGIN));
            ...
            protocol.setSubProtocol(SubProtocol.HANDSHAKE, true, event.getSession());
            event.getSession().send(new HandshakePacket(210, event.getSession().getHost(), event.getSession().getPort(), HandshakeIntent.STATUS));
