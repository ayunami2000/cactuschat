...
public class ServerListener extends SessionAdapter
{
    ...
    public void packetReceived(final PacketReceivedEvent event) {
            ...
            switch (packet.getIntent()) {
                ...
                case LOGIN: {
                    ...
                    if (packet.getProtocolVersion() > 210) {
                        event.getSession().disconnect("Outdated server! I'm still on 1.10.2.");
                        break;
                    }
                    if (packet.getProtocolVersion() < 210) {
                        event.getSession().disconnect("Outdated client! Please use 1.10.2.");
                        break;
                    }
                    break;
                }
        ...
        if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
            if (event.getPacket() instanceof LoginStartPacket) {
                ...
                final boolean verify = !event.getSession().hasFlag("verify-users") || event.getSession().getFlag("verify-users");
                if (verify) {
        ...
        if (protocol.getSubProtocol() == SubProtocol.STATUS) {
            if (event.getPacket() instanceof StatusQueryPacket) {
                ServerInfoBuilder builder = event.getSession().getFlag("info-builder");
                if (builder == null) {
    ...
    private class UserAuthTask implements Runnable
    {
        ...
        @Override
        public void run() {
            final boolean verify = !this.session.hasFlag("verify-users") || this.session.getFlag("verify-users");
            ...
            int threshold;
            if (this.session.hasFlag("compression-threshold")) {
                threshold = this.session.getFlag("compression-threshold");
            }
            else {
                threshold = 256;
            }
