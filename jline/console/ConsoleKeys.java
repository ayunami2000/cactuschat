...
public class ConsoleKeys
{
    ...
    protected boolean setKeyMap(final String name) {
        final KeyMap map = this.keyMaps.get(name);
        if (map == null) {
            return false;
        }
        this.keys = map;
                ...
                if (line.charAt(i) == '$') {
                    ...
                    if ("if".equalsIgnoreCase(cmd)) {
                        ...
                        if (args.startsWith("mode=")) {
                            if (args.equalsIgnoreCase("mode=vi")) {
                                ...
                                parsing = (args.equals("mode=emacs") && !this.isViEditMode());
                            }
                        }
                        else {
                    ...
                    if (keySeq.equalsIgnoreCase("set")) {
                        ...
                        if (val.length() > 0 && (val.charAt(0) == '\'' || val.charAt(0) == '\"')) {
                            ...
                            final String operationName = val.replace('-', '_').toUpperCase();
                            try {
