...
public class HttpPostMultipartRequestDecoder implements InterfaceHttpPostRequestDecoder
{
    ...
    public HttpPostMultipartRequestDecoder(final HttpDataFactory factory, final HttpRequest request, final Charset charset) {
        ...
        this.setMultipart(((ConvertibleHeaders<AsciiString, String>)this.request.headers()).getAndConvert(HttpHeaderNames.CONTENT_TYPE));
    ...
    private InterfaceHttpData findMultipartDisposition() {
        ...
        while (!this.skipOneLine()) {
            ...
            if (HttpHeaderNames.CONTENT_DISPOSITION.equalsIgnoreCase(contents[0])) {
                ...
                this.currentFieldAttributes.put(HttpHeaderNames.CONTENT_TRANSFER_ENCODING.toString(), attribute2);
            }
                ...
                this.currentFieldAttributes.put(HttpHeaderNames.CONTENT_LENGTH.toString(), attribute2);
            }
            else {
                ...
                if (HttpHeaderValues.MULTIPART_MIXED.equalsIgnoreCase(contents[1])) {
                    ...
                    for (int j = 1; j < contents.length; ++j) {
                        if (contents[j].toLowerCase().startsWith(HttpHeaderValues.CHARSET.toString())) {
                            ...
                            this.currentFieldAttributes.put(HttpHeaderValues.CHARSET.toString(), attribute3);
                        }
                        else {
    ...
    protected InterfaceHttpData getFileUpload(final String delimiter) {
        final Attribute encoding = this.currentFieldAttributes.get(HttpHeaderNames.CONTENT_TRANSFER_ENCODING.toString());
        ...
        final Attribute charsetAttribute = this.currentFieldAttributes.get(HttpHeaderValues.CHARSET.toString());
        if (charsetAttribute != null) {
            try {
                localCharset = Charset.forName(charsetAttribute.getValue());
            }
