...
public abstract class HttpContentEncoder extends MessageToMessageCodec<HttpRequest, HttpObject>
{
    ...
    protected void decode(final ChannelHandlerContext ctx, final HttpRequest msg, final List<Object> out) throws Exception {
        CharSequence acceptedEncoding = ((Headers<AsciiString>)msg.headers()).get(HttpHeaderNames.ACCEPT_ENCODING);
                ...
                case AWAIT_HEADERS: {
                    ...
                    if (isPassthru(res)) {
                        ...
                        if (result == null) {
                            ...
                            res.headers().set((CharSequence)HttpHeaderNames.CONTENT_ENCODING, (CharSequence)result.targetContentEncoding());
                            ((Headers<AsciiString>)res.headers()).remove(HttpHeaderNames.CONTENT_LENGTH);
                            res.headers().set((CharSequence)HttpHeaderNames.TRANSFER_ENCODING, (CharSequence)HttpHeaderValues.CHUNKED);
                            if (isFull) {
                                final HttpResponse newRes = new DefaultHttpResponse(res.protocolVersion(), res.status());
                                newRes.headers().set((TextHeaders)res.headers());
                                out.add(newRes);
                                break Label_0414;
                            }
                            out.add(res);
    ...
    protected abstract Result beginEncode(final HttpResponse p0, final CharSequence p1) throws Exception;
