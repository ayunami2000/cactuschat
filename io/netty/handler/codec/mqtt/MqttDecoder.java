...
public class MqttDecoder extends ReplayingDecoder<DecoderState>
{
        ...
        switch (this.state()) {
            ...
            case READ_PAYLOAD: {
                try {
                    ...
                    this.payload = ((Result<Object>)decodedPayload).value;
                    ...
                    final MqttMessage message = MqttMessageFactory.newMessage(this.mqttFixedHeader, this.variableHeader, this.payload);
    ...
    private static Result<MqttConnAckVariableHeader> decodeConnAckVariableHeader(final ByteBuf buffer) {
        ...
        final MqttConnAckVariableHeader mqttConnAckVariableHeader = new MqttConnAckVariableHeader(MqttConnectReturnCode.valueOf(returnCode));
    ...
    private static Result<ByteBuf> decodePublishPayload(final ByteBuf buffer, final int bytesRemainingInVariablePart) {
        final ByteBuf b = buffer.readSlice(bytesRemainingInVariablePart).retain();
    ...
    private static Result<String> decodeString(final ByteBuf buffer, final int minBytes, final int maxBytes) {
        ...
        final ByteBuf buf = buffer.readBytes(size);
        numberOfBytesConsumed += size;
