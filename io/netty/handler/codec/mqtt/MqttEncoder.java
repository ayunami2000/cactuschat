...
{
    public static final MqttEncoder DEFAUL_ENCODER;
    ...
    private static ByteBuf encodeConnectMessage(final ByteBufAllocator byteBufAllocator, final MqttConnectMessage message) {
        ...
        final byte[] willTopicBytes = (willTopic != null) ? encodeStringUtf8(willTopic) : MqttEncoder.EMPTY;
        final String willMessage = payload.willMessage();
        final byte[] willMessageBytes = (willMessage != null) ? encodeStringUtf8(willMessage) : MqttEncoder.EMPTY;
        if (variableHeader.isWillFlag()) {
            payloadBufferSize += 2 + willTopicBytes.length;
            payloadBufferSize += 2 + willMessageBytes.length;
        }
        final String userName = payload.userName();
        final byte[] userNameBytes = (userName != null) ? encodeStringUtf8(userName) : MqttEncoder.EMPTY;
        if (variableHeader.hasUserName()) {
            payloadBufferSize += 2 + userNameBytes.length;
        }
        final String password = payload.password();
        final byte[] passwordBytes = (password != null) ? encodeStringUtf8(password) : MqttEncoder.EMPTY;
    ...
    static {
        DEFAUL_ENCODER = new MqttEncoder();
        EMPTY = new byte[0];
    }
}
