...
public final class ByteBufUtil
{
    ...
    public static String hexDump(final byte[] array, final int fromIndex, final int length) {
        ...
        final char[] buf = new char[length << 1];
        for (int srcIdx = fromIndex, dstIdx = 0; srcIdx < endIndex; ++srcIdx, dstIdx += 2) {
            System.arraycopy(ByteBufUtil.HEXDUMP_TABLE, (array[srcIdx] & 0xFF) << 1, buf, dstIdx, 2);
        }
        return new String(buf);
    }
    ...
    public static int writeUtf8(final ByteBuf buf, final CharSequence seq) {
        ...
        if (buf instanceof AbstractByteBuf) {
            final AbstractByteBuf buffer = (AbstractByteBuf)buf;
            int writerIndex;
            final int oldWriterIndex = writerIndex = buffer.writerIndex;
            for (int i = 0; i < len; ++i) {
            ...
            return (buffer.writerIndex = writerIndex) - oldWriterIndex;
        }
    ...
    public static int writeAscii(final ByteBuf buf, final CharSequence seq) {
        if (buf == null) {
            throw new NullPointerException("buf");
        }
        if (seq == null) {
